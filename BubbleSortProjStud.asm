...
; the new sorted list of numbers.
	...
	LD R1, FILE		; Set R1 as the pointer to the memory address of the digits list FILE
	AND R2, R2, #0		; Clear R2 prepare for storing first digit 1 read from the FILE
...
Sort	LDR R2, R1, #0		; Loads first digit value 1 from the memory address where R1 pointed to into R2
	...
	ADD R4, R4, #1		; Makes R3 negative of itself for subtraction
...
OutputData			; Subroutine 1: Outputs data starting at x4000
	...
	ADD R3, R3, R7		; Saves return address to R3
...
EndLoop	LD R0, newline
	...
	RET			; Returns to main program when loop ends
...
Bin_to_ASCII			; Subroutine 2: Converts binary num to a string placed at x4500
	...
	LD R1, ASCIIString      ; Set R1 point to memory address ASCIIString will be used saved the output (usorted or sorted) string.
...
Begin1	STR R2, R1, #0          ; Write the '+'/'-' in R2 to memory address R1 pointed to with offset #0
	LD R2, ASCIIoffset	; R2 <= character '0' (for ones digit)
	ADD R4, R0, R2		; Convert the binary digit in R0 to char into R4
	...
	LD R0, SR0              ; Restore R0's data from save address memory before return to caller.
	LD R1, SR1              ; Restore R1's data from save address memory before return to caller.
	LD R2, SR2              ; Restore R2's data from save address memory before return to caller.
	LD R3, SR3              ; Restore R3's data from save address memory before return to caller.
